...
/// <param name="ToSave">The items to write: the ones missing here or newer in the backup.</param>
public sealed record MergeResult<T>(IReadOnlyList<T> ToSave, int Added, int Updated, int Skipped) where T : Entity;
...
public static class BackupMerge
{
    public static MergeResult<T> Merge<T>(IEnumerable<T> existing, IEnumerable<T> incoming) where T : Entity
    {
        ...
        return new MergeResult<T>(save, added, updated, skipped);
    }
}
